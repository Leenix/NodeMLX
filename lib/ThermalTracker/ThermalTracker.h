...
const int MAX_DISTANCE_THRESHOLD = 300;
const int RUNNING_AVERAGE_SIZE = 800;
const int REFRESH_RATE = 16;
const int UNCHANGED_FRAME_DELAY = REFRESH_RATE * 3;
...
class ThermalTracker {
   public:
    ThermalTracker(int _running_average_size = RUNNING_AVERAGE_SIZE,
                   int _max_distance_threshold = MAX_DISTANCE_THRESHOLD, int _min_blob_size = MINIMUM_BLOB_SIZE);
    ...
    void generate_distance_matrix(TrackedBlob tracked_blobs[MAX_BLOBS], Blob blobs[MAX_BLOBS],
